...
//   - admin_static:    holds the admin app's `_next/static/` chunks, served via
//                      `admin_static` pull zone and referenced at build time
//                      through `ADMIN_ASSET_PREFIX` (iter-41). Uploads are
//                      additive (`remove: false`) so older container instances
//                      can still resolve their chunks during a rolling deploy.
