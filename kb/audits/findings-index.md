...
| F-FE-30 | Missing semantic shadcn tokens (`--popover`, `--accent`, `--destructive-foreground`) | 16h | 🟢 (`@shadcn/theme-neutral` wholesale; all semantic tokens now declared) |
| F-FE-31 | User menu missing from sidebar | 16h | 🟢 (`UserMenu` in sidebar footer via `@shadcn/sidebar-07` graft) |
| F-FE-32 | No visual hierarchy on dashboard home | 16h | 🟢 (KPI grid + chart placeholder + recent-events per `dashboard-01` layout shape) |
