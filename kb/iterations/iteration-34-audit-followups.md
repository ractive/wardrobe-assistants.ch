...
- **Deferred to a separate iteration** (intentionally not in scope):
  ...
  - **`kb/admin-architecture/` wiki restructure.** Pending memory note (`project_kb_restructure_pending`) — split `design-system/README.md` into per-topic pages linked from `overview.md`. Worth doing soon to shrink agent context usage, but doc-only and parallel-safe; can land any time post iter-32.
