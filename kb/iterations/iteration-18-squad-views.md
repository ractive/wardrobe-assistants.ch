...
## Scope — squad routes [0/4]

- [ ] Route `app/(dashboard)/my-events/page.tsx`:
...
- [ ] Route `app/(dashboard)/upcoming-events/page.tsx`:
...
- [ ] Sidebar shows different links per role:
  - ADMIN: Users / Events / Services (admin views).
  - SQUAD_MEMBER: My Events / Upcoming Events.
- [ ] Dashboard landing (`/`) routes by role: ADMIN → admin dashboard; SQUAD_MEMBER → `/my-events`.

## Scope — request-to-participate [0/3]

- [ ] `requestParticipation` action in `features/events/server/actions.ts`:
...
- [ ] `approveRequest` / `rejectRequest` actions:
...
- [ ] Admin events list shows pending requests as a badge; event detail page has approve/reject buttons inline.

## Scope — UI [0/3]

- [ ] `<MyEventsTable>` (squad-side) — read-only, shows event name/date/venue + status of the assignment.
- [ ] `<UpcomingEventsList>` (squad-side) — card layout per event with a "Request to participate" button.
- [ ] `<RequestsBadge>` on the admin events table — shows count of pending requests.

## Scope — verify [0/3]

- [ ] Tests: perm enforcement (squad can't reach admin routes; admin can't accidentally request on their own events); the "already assigned" check; email fan-out on request.
...
- [ ] SQUAD_MEMBER sees only `/my-events` and `/upcoming-events`; admin sidebar surfaces are 404 / forbidden for them.
- [ ] SQUAD_MEMBER can request participation on an upcoming event; admin gets notified.
- [ ] Admin can approve or reject; on approve, squad-member sees it in "Assigned to you."
- [ ] `npm run verify` green.
