...
7. **Critical operational footgun discovered: `BunnyWay/actions/container-update-image@main` strips `environmentVariables` from the container template when patching the image_tag.** During iter-9, the post-cutover admin endpoint started returning HTTP 500 from `/api/auth/sign-in/email`. Investigation showed `containerTemplates[0].environmentVariables: []` — empty after a successful BunnyWay roll. Re-setting the env (via `hoppy container template env`) and pod recreate fixed it. **The user (operator)** confirmed corroborating prior context: bunny's MC PATCH endpoint has a known issue where partial template updates clobber unsubmitted fields including the entire `environmentVariables` array. So this is the underlying API behaviour, not just a BunnyWay action bug — any client that does `PATCH /mc/apps/{id}/container-templates/{id}` without echoing back the existing env array will wipe it.
   **Action items:**
   - Audit `hoppy container template update` and `hoppy container app update` for the same hazard (likely also affected — same API).
   - File an issue at https://github.com/BunnyWay/actions asking that `container-update-image` GET the template before PATCH and merge `environmentVariables` back in.
   - In the meantime: a CI step after the BunnyWay action that re-asserts env vars (e.g. `hoppy container template env --env ...` reading from GitHub secrets) closes the loop. This means lifting `DATABASE_URL`, `DATABASE_AUTH_TOKEN`, `BETTER_AUTH_SECRET`, `RESEND_API_KEY`, etc. into GitHub secrets so CI can replay them. The container-template-as-source-of-truth model breaks under this PATCH bug; the durable answer is GitHub-secrets-as-source-of-truth + a CI assert-env step.
