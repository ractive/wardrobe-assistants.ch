...
- [x] Set `metadataBase` in `src/app/layout.tsx` to the production URL (`https://wardrobe-assistants.ch`)
- [x] Add a `metadata` export with:
...
- [x] Set `<html lang="en">` (already correct — verify only)
- [x] Create the OG image:
...
- [x] Add favicon set: 32×32 PNG, 180×180 apple-touch-icon, plus the existing `favicon.ico`. (Next 16 picks these up by file convention in `app/`.)
- [x] Verify with the Open Graph debugger and Twitter card validator after deploy
